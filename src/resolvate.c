...
int main(int argc,char *argv[]) {
	...
	if(argc<4) {
		printf("usage: ./resolvate.exe input.pdb input.cube dist\n");
	...
	readPDBcrd(fnPDB,&prot);
	/*boil down list of maxima to list of maxima in range of protein*/
	j=0;
	for(l=0;l<nMax;l++) {
		i=0;
		dSq=minDSq;
		while(i<prot.nAtoms && dSq>=minDSq) {
			distSq(gridCrd[l],prot.atCrd[i],&dSq);
			i++;
		}
		if(dSq<minDSq) {
			sorted[j]=sorted[l];
	...
	for(l=0;l<nMax;l++) {
		fprintf(io,"HETATM%5d %4s %-4s %4d    %8.3f%8.3f%8.3f%6.2f%6.2f\n",
			l+1+prot.nAtoms," O  ","HOH",l+1,gridCrd[l][0],gridCrd[l][1],gridCrd[l][2],sums[l],1.0);
	}
	fclose(io);
	return 0;
}
